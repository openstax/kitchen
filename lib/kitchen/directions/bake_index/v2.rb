  ...
  class V2
    ...
    def bake(book:, types:)
      ...
      types.each do |type|
        ...
        bake_single_index(book: book, type: @type)

        book.first('body').append(child: render(file: 'v2.xhtml.erb'))
      end
    end

    def bake_single_index(book:, type:)
      selector = type == 'term' ? '$:not([cxlxt|index])' : "$[cxlxt|index='#{@type}']"

      book.pages.terms(selector.to_s).each do |term_element|
        page = term_element.ancestor(:page)
        page_title = page.title.text
        add_term_to_index(term_element, page_title)
      end

      book.chapters.composite_pages.terms(selector.to_s).each do |term_element|
