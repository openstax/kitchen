...
module Kitchen
  module Directions
    module BakeFootnotes
      ...
      # Helpers
      ROMAN_NUMERALS = %w[i ii iii iv v vi vii viii ix x].freeze

      def self.as_roman(number)
        index = number - 1
        raise 'Unknown conversion to Roman numerals' if index >= ROMAN_NUMERALS.size

        ROMAN_NUMERALS[index]
      end

      def self.bake_footnotes_within(container, roman: false)
        footnote_count = 0
        aside_id_to_footnote_number = {}

        container.search("a[role='doc-noteref']").each do |anchor|
          footnote_count += 1
          footnote_number = roman ? as_roman(footnote_count) : footnote_count.to_s
